<?php
	...
	try{
		...
		define('max_item', 12);
		//必要なページ数を求める
		$count = $pdo->prepare('SELECT COUNT(*) AS count FROM t_book'.$where);
		$count->execute();
		...
		if($now ==1){
   			//1ページ目の処理
    			...
    			$select->bindValue(':max', max_item, PDO::PARAM_INT);
		}else{
    			//1ページ目以外の処理
    			$select->bindValue(':start', ($now-1) * max_item, PDO::PARAM_INT);
    			$select->bindValue(':max', max_item, PDO::PARAM_INT);
		}
		//実行し結果を取り出す
		$select->execute();
...
　　 <div class="book">
	...
	if($now == 1){	
                ...
                echo "<a href='./index.php?{$querySelectInput}{$querySort}page_id=1'
	        style='padding:5px 10px;font-size:18px;border:1px solid #000;text-decoration:none;color:#000;'>1</a>";
	}
	if($now>=4){
		echo "・・・";
	}
	    ...
	    if($n==$now){
                ...
                echo "<a href='./index.php?{$querySelectInput}{$querySort}page_id=$n'
                style='padding:5px 10px;font-size:18px;border:1px solid #000;text-decoration:none;color:#000;'>{$n}</a>";
            }else{
		continue;
	    }
        }
	...
	if($pages == 1){
        	...
        	echo "<a href='./index.php?{$querySelectInput}{$querySort}page_id=$pages'
	        style='padding:5px 10px;font-size:18px;border:1px solid #000;text-decoration:none;color:#000;'>{$pages}</a>";
	}    
        ?>
    </section>
    </main>
