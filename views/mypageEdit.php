...
<?php
	/* ---------------------------------------------------------------------
		...
		$userData = $db->selectfetchAll($sql,[[':userId', $userId, PDO::PARAM_STR]]); 
    	} catch(Exception $e) {
...
<?php
		...
		$db = new DbConnect(getenv("DB_USERNAME"), getenv("DB_PASSWORD"), getenv("DB_DATABASE"), getenv("DB_HOST"));
		$db->createPdo();
        	$sql = 'SELECT t_good.no, t_book.imageUrl, t_book.bookTitle FROM t_good INNER JOIN t_users ON t_good.userId = t_users.userId INNER JOIN t_book ON t_book.no = t_good.no WHERE t_users.userId = :userId';
		$goodData = $db->selectfetchAll($sql,[[':userId', $userId, PDO::PARAM_STR]]); 
      	} catch(Exception $e) {
        	echo '<span class="error">エラーがありました。</span><br>';
        	echo $e->getMessage().'<br>';
      	}
...
<?php
		...
		$db = new DbConnect(getenv("DB_USERNAME"), getenv("DB_PASSWORD"), getenv("DB_DATABASE"), getenv("DB_HOST"));
		$db->createPdo();
        	$sql = 'SELECT t_review.reviewId, t_review.no, t_book.imageUrl, t_book.bookTitle, t_review.reviewContent FROM t_review INNER JOIN t_users ON t_review.userId = t_users.userId INNER JOIN t_book ON t_review.no = t_book.no WHERE t_users.userId = :userId';
		$reviewData = $db->selectfetchAll($sql,[[':userId', $userId, PDO::PARAM_STR]]); 
      	} catch(Exception $e) {
        	echo '<span class="error">エラーがありました。</span><br>';
        	echo $e->getMessage().'<br>';
      	}
	...
	<body>
  		<!-- モバイル用ドロワーメニューここから -->
        	<div class="drawer">
            		...
            		<div class="cancelDrawerLayer"></div>
        	</div>	
  		<!-- モバイル用ドロワーメニューここまで -->
        			...
        			<div class="flexContainer-Profile-outline">
            				<div class="flexContainer-Profile">
                				<div class="flexItem-PhotoName">
                    					<label>
                        					<img src='../images/userphoto/userDefault.jpg' class="userphoto userphoto1 <?php echo isset($userData[0]['imageUrl']) && !empty($userData[0]['imageUrl'])? 'displayNone': '' ?>" alt="プロフィール写真">
                        					<img src='<?php echo isset($userData[0]['imageUrl']) && !empty($userData[0]['imageUrl'])? $image : "../images/userphoto/cameraIcon.jpeg"?>' class="userphoto userphoto2" alt="カメラの写真">
	            						<input type="hidden" name="MAX_FILE_SIZE" value="500000">
		    						<input type="file" name="file">
                    					</label><br>
                    					<label>
								<input type="text" name="name" value="<?php echo $userData[0]['name']? es($userData[0]['name']) : '名前未設定'; ?>"> さん
							</label>
                				</div>
                				<div class="flexItem-Detail">
                    					<label>
								<span class="email">メールアドレス：</span><br>
								<input type="email" name="email" value="<?php echo es($userData[0]['email']); ?>">
							</label>
                    					<p class="errorText"><?php echo $_SESSION['validationErrors']['checkEmail'];?></p>
							<div class="<?php echo $userId === 'testUser1'? 'invisible' : ''?>">
		    						<label>
									<span class="password">新しいパスワード：</span><br>
									<input type="password" name="password" placeholder="8文字以上半角英数(大文字含)">
                    						...
                    						<label>
									<span class="passwordConfirm">確認用パスワード：</span>
									<input type="password" name="passwordConfirm" placeholder="8文字以上半角英数(大文字含)">
								</label>
									...
									<input type="checkbox"><span class="check">パスワードを表示する</span>
                    					...
                    					<label>
								<span class='experience'>実務経験：</span><br>
								<input type="number" name="workYear" value="<?php echo es($userData[0]['workYear']) ?>"> <span class="experienceYear">年</span>
							</label>
                    					<p class="errorText"><?php echo $_SESSION['validationErrors']['isNumber']; ?></p>
                    					<label>
								<span class="language">得意な言語：</span><br>
								<input type="text" name="language" value="<?php echo es($userData[0]['language']) ?>">
							</label><br>
                    					<label>
								<span class="comment">コメント：</span><br>
								<textarea name="comment"><?php echo es($userData[0]['comment']) ?></textarea>
							</label>
                				</div>
            				</div>
        			</div>	
            					...
            					<?php foreach($goodData as $goodDatum) : ?>
            						<div class="goodBookFlexItem" data-id="<?php echo $userId; ?>" data-no="<?php echo es($goodDatum['no']); ?>">
                						...
                						<a href="./bookDetail.php?bookNo=<?php echo es($goodDatum['no']); ?>">
                    							<img src="<?php echo es($goodDatum['imageUrl']); ?>" alt="本の画像">
                						</a>
            						</div>
            					<?php endforeach;?>
            						...
            						<div class="reviewBookFlexItem">
                						<div class="bookItem">
                    							...
                    							<a href="./bookDetail.php?bookNo=<?php echo es($reviewDatum['no']); ?>">
                        							<img src="<?php echo es($reviewDatum['imageUrl']); ?>" alt="本の画像">
                    							</a>
                						</div>
