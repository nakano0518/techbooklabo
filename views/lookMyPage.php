...
<?php
    ...
    try {
        ...
        $select->bindValue(':followId', $followId, PDO::PARAM_INT);
        $select->execute();
...
<?php
    ...
    -----------------------------------------------------------------------*/
      try {
        ...
        INNER JOIN t_book ON t_book.no = t_good.no WHERE t_users.followId = :followId';
        $select = $pdo->prepare($sql);
        $select->bindValue(':followId', $followId, PDO::PARAM_INT);
        $select->execute();
...
<?php
    ...
    -----------------------------------------------------------------------*/
      try {
        ...
        INNER JOIN t_book ON t_review.no = t_book.no WHERE t_users.followId = :followId';
        $select = $pdo->prepare($sql);
        $select->bindValue(':followId', $followId, PDO::PARAM_INT);
        $select->execute();
