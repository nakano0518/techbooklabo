...
//検索キーワードをエンジニア分野に限定するため、ホワイトリストの作成
	...
	if(isset($pages)&&!empty($pages)) {
		...
		for($n=2; $n<=$pages-1;$n++){
                	if($n==$now){
                    		...
                    		echo "<a href='./bookInsert.php?bookKeywords=$valuePlusConnect&page_id=$n'
                    		style='padding:5px 10px;font-size:18px;border:1px solid #fff;text-decoration:none;color:#fff;margin-right:5px;'>{$n}</a>";
                	}else{
				continue;
			}
            	}
		...
		if($pages == 1){
                    	...
                    	echo "<a href='./bookInsert.php?bookKeywords=$valuePlusConnect&page_id=$pages'
                    	style='padding:5px 10px;font-size:18px;border:1px solid #fff;text-decoration:none;color:#fff;margin-right:5px;'>{$pages}</a>";
		}
        }
        ?>
    </section>
